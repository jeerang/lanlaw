...
$pageTitle = 'เพิ่มสำนักงานเจ้าของงาน';
$tableName = 'offices';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $db = getDB();
            $db->prepare("INSERT INTO {$tableName} (code, name, short_name, address, phone, contact_person, status) VALUES (?, ?, ?, ?, ?, ?, ?)")->execute([$data['code'], $data['name'], $data['short_name'], $data['address'], $data['phone'], $data['contact_person'], $data['status']]);
