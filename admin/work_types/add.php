...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $db = getDB();
            $stmt = $db->prepare("INSERT INTO {$tableName} (code, name, description, status) VALUES (?, ?, ?, ?)");
            $stmt->execute([$data['code'], $data['name'], $data['description'], $data['status']]);
            $id = $db->lastInsertId();
            logActivity('create', $tableName, $id, null, $data);
...
<div class="card">
    <div class="card-header">
        <i class="fas fa-plus-circle me-2"></i><?php echo $pageTitle; ?>
    </div>
    <div class="card-body">
