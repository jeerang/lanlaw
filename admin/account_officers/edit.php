...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $db = getDB();
            $db->prepare("UPDATE {$tableName} SET code=?, name=?, department=?, phone=?, email=?, status=? WHERE id=?")->execute([$data['code'], $data['name'], $data['department'], $data['phone'], $data['email'], $data['status'], $id]);
