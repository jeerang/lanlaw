...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $db = getDB();
            $db->prepare("UPDATE {$tableName} SET code=?, name=?, description=?, status=? WHERE id=?")->execute([$data['code'], $data['name'], $data['description'], $data['status'], $id]);
